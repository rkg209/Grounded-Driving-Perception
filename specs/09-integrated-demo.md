...
question → see an answer** (Stage 2). This is what gets recorded as the demo video and linked from
the resume.
...
1. `gdp demo` launches; both panels work end-to-end on a preloaded scene.
2. The "two separate models" caption is present in the UI.
3. Out-of-taxonomy query results are visibly labelled as unmeasured demos.
4. Latency shown is real, measured at request time — never hardcoded.
5. A demo video is recorded showing both panels and the threshold slider.

## Honesty contract

- **H6** — **the highest-risk spec for this rule.** A single "ask anything about this scene" box
  that silently routes to two models would imply a joint system. Keep the panels separate and say
  why.
